...
export default function ProductItem(props) {
  ...
  return (
    <Link href={`/product/${_id}`}>
      <div className="w-60 overflow-hidden transition-all duration-300 shadow-md bg-opacity-30 bg-gray-100 rounded-md p-2 hover:shadow-xl">
        <div className="flex items-center justify-center overflow-hidden p-10 h-40">
          <Image src={img} alt={title} width={128} height={128} className="img transition-all duration-300 hover:scale-90" />
        </div>
        <div className="flex flex-col p-2 gap-1">
          <h2 className="text-xl text-gray-700 overflow-ellipsis overflow-hidden whitespace-nowrap">{title}</h2>
          <p className="text-xs text-gray-500 overflow-ellipsis overflow-hidden whitespace-nowrap">{desc ? desc : "No Description"}</p>
