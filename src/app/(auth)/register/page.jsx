...
import { signUpSucces, signupFailed } from "@/lib/features/user/userSlilce";
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length === 0 && isSubmit) {
      try {
        const res = await publicRequest.post("api/auth/register", formValues);
        ...
        <form className="space-y-4" onSubmit={handleSubmit} autoComplete="on">
          ...
          <div>
            <input
              ...
              onChange={handleOnChange}
            />
            <span className="w-100 text-sm text-red-600 mb-[1px]">{formErrors.cpassword}</span>
          </div>
          <button className="bg-black text-white py-2 px-4 rounded focus:outline-none focus:shadow-outline" disabled={isFetching}>
